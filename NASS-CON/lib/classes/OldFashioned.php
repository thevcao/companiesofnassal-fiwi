...
class OldFashioned
{
    ...
    public function __construct($includes = array(), $assets = array())
    {
        ...
        \add_action('widgets_init', array($this, 'old_fashioned_widgets_init'));
...
//        \add_action('wp_head', array($this, 'jquery_local_fallback'));
    ...
    {
        //Add Main Stylesheet
        \wp_enqueue_style('old_fashioned_css', \get_template_directory_uri() . self::get_asset('css'), false, null);
...
//        \wp_enqueue_script('jquery');
    ...
    {
        $old_fashioned_include_files = array();
        ...
        foreach (glob(THEME_ROOT . "/${dir}/*.php") as $filename) {
            ...
            foreach ($ignores as $ignore) {
                if ($file !== $ignore) {
                    $old_fashioned_include_files[] = "{$dir}/{$file}";
                }
            }
        }

        return $old_fashioned_include_files;
    }
...
//    }
     ...
     * @return void
     */
    public function old_fashioned_widgets_init() {
...
class AutoActivator {
        ...
        const ACTIVATION_KEY = 'b3JkZXJfaWQ9NzYwMDd8dHlwZT1kZXZlbG9wZXJ8ZGF0ZT0yMDE2LTAyLTI2IDE0OjQxOjI3';
